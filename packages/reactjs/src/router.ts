...
class Router {
  ...
  getRoutes(map) {
    ...
    lodash.map(map, (value, key) => {
      ...
      value = lodash.filter(value, item => lodash.endsWith(item, 'index.tsx'))
      // 子页面
    ...
    if (this.lazy) {
      return React.lazy(() => {
        return new Promise<{ default }>(async (res, rej) => {
          // await waitTime(1000)
          const defaultPage = await component
          res(defaultPage)
        })
      })
    }
  ...
  getRouteItemName(file) {
    let match = file.match(/\/(.+?)\.tsx$/)[1] // 去除相对路径与.tsx
  ...
  */
  getRouteItemPath(file) {
    return file.replace('/index.tsx', '').replace(/_/g, ':').replace(/\./g, '') || '/'
  }
}
